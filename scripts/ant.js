class Ant{
    ...
    hitBorder(){
        if(this.hitHorizontalBorder())
            this.direction = 90 - (180 - 90 - (0 - this.direction));

        if(this.hitVerticalBorder())
            this.direction = 90 - (180 - 90 - (180 - this.direction));
    }
    ...
    isInTheBorders(area){
        return this.position.x > area.position.x - area.width / 2 && this.position.x < area.position.x + area.width / 2 &&
                this.position.y > area.position.y - area.width / 2 && this.position.y < area.position.y + area.width / 2;
    }
    ...
    searchForPheromones(type, base, angle = this.pheromoneAngle, maxDistance = this.pheromoneDistance){
        ...
        for(let i=0; i<angle; i+=3){
            for(let distance=5; distance<maxDistance; distance+=3){
                ...
                if(base.position.dist(this.position) < 20) foundPheromones.add({force: foodPheromonesMaxForce + 10, position: base.position});
